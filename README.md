...
* Cómo combinar la inteligencia artificial con videojuegos con [Gymnasium](https://gymnasium.farama.org/) (el fork mantenido de OpenAI Gym) para aprender de forma efectiva.
...
La forma rápida y recomendada es con [`uv`](https://github.com/astral-sh/uv):
...
Si prefieres `pip` o `conda`, o necesitas detalles (Atari, MuJoCo, GPU, WSL,
notebooks, troubleshooting…), tienes la guía completa en
[`SETUP.md`](SETUP.md).

Ver [`MIGRATION.md`](MIGRATION.md) para los cambios respecto al `gym` v0.21
que se usaba originalmente en el curso (la API de `step`/`reset`, los IDs
de Atari, etc.).
